...
export async function POST(request) {
  try {
    ...
    if (!isGeminiConfigured()) return jsonError("Bill scanning is not configured", 503);
    ...
    try {
      ...
      if (error && error.message === "UNREADABLE") {
        return jsonError("Could not read this bill. Please try a clearer photo.", 502);
      }
      return jsonError("Bill scanning failed. Please try again.", 502);
    }

    if (!scan.items.length) {
      return jsonError("No items could be read from this bill. Please try a clearer photo.", 422);
    }
